 ...
 * @brief 处理双目图像并跟踪当前帧。
 * 
 * 此函数接收校正后的双目图像，如果需要会将其转换为灰度图像，并初始化一个新帧用于跟踪。
 * 它支持不同的传感器配置，例如双目和IMU-双目。函数还会为当前帧分配元数据，如文件名和数据集编号，并执行跟踪。
 * 
 * @param imRectLeft 校正后的左图像 (cv::Mat)。
 * @param imRectRight 校正后的右图像 (cv::Mat)。
 * @param timestamp 与双目图像关联的时间戳。
 * @param filename 当前帧的文件名。
 * @return Sophus::SE3f 当前帧跟踪后的位姿。
 */
