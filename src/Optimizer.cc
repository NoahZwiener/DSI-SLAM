...
namespace ORB_SLAM3
{
    ...
    int Optimizer::PoseOptimization(Frame *pFrame)
    {
        ...
        // std::cout << "sum figure N=%d" << N<< std::endl;
        ...
        for (int i = 0; i < pFrame->N; i++)
        {
            ...
            // 仅统计有效跟踪的内点 (Inliers)
            if (pMP && !pMP->isBad() && pFrame->mvbOutlier[i] == false)
            {
                // 累加该特征点在提取阶段算出的四叉树显著性
                S_sum += pFrame->mvSaliencySpatial[i];
                tracked_count++;
            }
        }

        // 参数定义 (需要根据你实际打印的 S_sum 范围进行标定)
        float alpha = 0.004f; // 曲线斜率
        float S_th = 1400.0f; // 突变阈值中点
        // 计算当前帧的原始显著性
        float raw_saliency = 1.0f / (1.0f + std::exp(-alpha * (S_sum - S_th)));

        // 2. 强时域滤波器 (EMA) - 这一步是拉出长趋势线的关键！
        // 注意：需要在 Tracking.h 中定义 float mLastSaliencyVisual; 并在构造函数中初始化为 1.0f
        float beta = 0.85f; // 平滑系数 (0.85 意味着极强的物理惯性，你可以根据画图效果在 0.8 到 0.95 之间微调)
        pFrame->mSaliencyVisual = beta * pFrame->mLastSaliencyVisual + (1.0f - beta) * raw_saliency;

        // 更新历史状态
        pFrame->mLastSaliencyVisual = pFrame->mSaliencyVisual;
        // 计算帧级视觉显著性 S_visual
        // pFrame->mLastSaliencyVisual = 1.0f / (1.0f + std::exp(-alpha * (S_sum - S_th)));

        // 边界限幅，确保 EKF 协方差计算不会除以零或溢出
        if (pFrame->mSaliencyVisual > 0.95f)
            pFrame->mSaliencyVisual = 0.95f;
        if (pFrame->mSaliencyVisual < 0.05f)
            pFrame->mSaliencyVisual = 0.05f;

        // std::cout << "Tracked Inliers: " << tracked_count << " | S_sum: " << S_sum << std::endl;
        //  ================== 新增部分结束 ==================
#endif
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges)
    {
        ...
        double coefficient_spatial;

        cv::FileStorage fs;
        if (!fs.open("/home/nuc/orb_slam3/ORBSLAM/ORBSLAM/custom.yaml", cv::FileStorage::READ))
        {
            std::cerr << "错误：无法打开或找到YAML配置文件 'custom.yaml'" << std::endl;
            return;
        }

        // 直接读取完整键 "Optimizer.Coefficient_spatial"
        if (!fs["Optimizer.Coefficient_spatial"].empty())
        {
            fs["Optimizer.Coefficient_spatial"] >> coefficient_spatial;
        }
        else
        {
            coefficient_spatial = 0.8; // 设置默认值
            std::cerr << "警告：读取 'Optimizer.Coefficient_spatial' 失败，使用默认值 0.8" << std::endl;
        }
        // 关闭文件
        fs.release();
#endif
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
            {
                ...
                if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
                {
                    ...
                    else if (leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)] >= 0) // Stereo observation
                    {
                        ...
                        const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];
#ifdef USE_SALIENCY
                        e->setInformation(Eigen::Matrix3d::Identity() * invSigma2 * final_weight);
#else
                        e->setInformation(Eigen::Matrix3d::Identity() * invSigma2);
#endif
                        g2o::RobustKernelHuber *rk = new g2o::RobustKernelHuber;
...
#else
                        e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
                        //cout<< "PoseInerOpt_weight" << Eigen::Matrix2d::Identity() *invSigma2 << endl;
#endif
                        // e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
...
#else
                        e->setInformation(Eigen::Matrix3d::Identity() * invSigma2);
                        //cout<< "PoseInerOpt_weight" << Eigen::Matrix3d::Identity() *invSigma2 << endl;
#endif
                        //e->setInformation(Eigen::Matrix3d::Identity() * invSigma2);
