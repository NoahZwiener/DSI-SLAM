...
Frame::Frame(const Frame &frame)
    : mpcpi(frame.mpcpi), mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
        ...
        mTcw(frame.mTcw), mbHasPose(false), mbHasVelocity(false), mvSaliencySpatial(frame.mvSaliencySpatial), mvSaliencySpatialRight(frame.mvSaliencySpatialRight)
{
#else
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
...
#ifdef USE_SALIENCY
    if(flag==0)
        monoLeft = (*mpORBextractorLeft)(im,cv::Mat(),mvKeys,mDescriptors,vLapping, mvSaliencySpatial);
    else
        monoRight = (*mpORBextractorRight)(im,cv::Mat(),mvKeysRight,mDescriptorsRight,vLapping, mvSaliencySpatialRight);
#else
    if(flag==0)
